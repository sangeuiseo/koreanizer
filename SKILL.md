...
**전:**
> - **성능:** 목록 스크롤이 부드러워졌다.
> - **보안:** 로그인 토큰을 자주 새로 고친다.
**후:**
> 목록 스크롤이 부드러워졌고, 로그인 토큰을 자주 새로 고친다.
**전 (한 줄):**
> **재시도**: 3회, 간격 2배
**후:**
...
**전:**
> 이 함수는 해시맵으로 찾는다. 모든 항목을 순회하던 이전 방식을 대체하기 위해 추가되었다.
**후:**
> 이 함수는 해시맵으로 찾는다.
...
기호가 서술어 자리를 차지한 곳을 푼다. 조사가 문법 역할을 표시하므로, 기호가 그 자리를 먹으면 독자가 관계를 추론해야 한다. `성능·안정성 개선`은 둘이 나란한지 앞이 뒤를 꾸미는지 말하지 않고, `캐시/세션 정리`는 "와"인지 "또는"인지 말하지 않으며, `요청 실패 → 재시도`는 차례인지 까닭인지 말하지 않는다. 괄호가 열다섯 자 넘는 절을 삼킨 자리도 같다. "성능과 안정성을 개선했다", "캐시와 세션을 지운다", "요청이 실패하면 다시 시도한다"처럼 조사와 서술어를 되살린다.

제목과 이름표는 그대로 둔다. 중점으로 이은 제목은 그 장르에서 오래 쓴 형태다. 관계를 원문으로 알 수 없으면 고르거나 이어 붙이지 않는다.
